...
#ifdef VCS_V5
CORN::Date_32_clad global_today;                                                 //150723LML note: used for CropSyst and include the spinup period
int global_rec;
...
int set_average_veglib_for_crop(veg_lib_struct &veglib,
                                const veg_var_struct * const *veg_var,
                                ...
                                const int current_month)
{
    ...
    //170630 int Ndist = options.NDIST;
    ...
    veglib.wind_h                        = 0;
    veglib.RGL                           = 0;
    //170630 for (int dist = 0; dist < Ndist; dist++ ) {                                  //150929LML
      for (int band = 0; band < options.SNOW_BAND; band++) {                                //150929LML
        crop_data_struct *current_crop = veg_var/*170630 [dist]*/[veg_index][band].VCS.crop_state;
        if (current_crop) {
          VIC_land_unit_activate(current_crop->CropSystHandle);                  //151001LML time consuming!!!
          double mu = 1.0;                                                       //170630 (dist == WET ? prcp->mu[veg_index] : (1.0 - prcp->mu[veg_index]));
          ...
          veglib.Wdmax[current_month]          += mu * snow_zone_frac * param.VEG_LAI_WATER_FACTOR * veglib.LAI[current_month];
                                           ...
                                           )
{
    ...
    double D                = irr_eff.D;                                         //3.0; //average droplet size mm
    double teta             = irr_eff.q_nuzzle * 0.0174532925;                   //35.0*0.0174532925; //angle of the nuzzle it should be converted to radian by a function
    double x0               = irr_eff.x_dm_spk;                                  //10.0; //diameter of sprinkler
    double y0_0             = irr_eff.y0_spk;                                    //2.0; //height of the irrigation sprinkler from ground m
    double time_of_irrig    = irr_eff.tirrig;
    double actual_drop_size = irr_eff.actual_droplet_size;
    double Ap               = irr_eff.A_Ap;                                      //0.07;//percentage of area which is covered by irrigation system at a time
    #else
...
{
    #if (VIC_CROPSYST_VERSION>=3)
    if((veg_class_code_in_veg_con_file >= VEG_CLASS_CODE_START_NONE_ROTATION_CROP &&
        veg_class_code_in_veg_con_file <= VEG_CLASS_CODE_END_ROTATION_CROP)
...
//_150526______________________________________________________________________/
bool IsRotationWithMultipleCycles(int veg_class_code_in_veg_con_file)
{
    if (veg_class_code_in_veg_con_file >= VEG_CLASS_CODE_START_ROTATION_CROP &&
        veg_class_code_in_veg_con_file <= VEG_CLASS_CODE_END_ROTATION_CROP)
        return true;
    else
        return false;
