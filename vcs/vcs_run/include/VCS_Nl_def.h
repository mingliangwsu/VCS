...
#define dist_prcp_struct all_vars_struct
#endif
...
#define BARE_SOIL_LIB_CODE                      (MAX_VEGCLASS_LIB_CODE + 1)
...
#endif
  char global_crop[MAXSTRING];                                                   //global crop control file name
...
typedef struct {
    ...
    bool clay_input;                                                               //*TRUE means clay percentage should be provided at the end of soil file*
    #if (VIC_CROPSYST_VERSION>=3)
...
typedef struct Irrigation_Types_In_Each_Cell {
  ...
  /*170518LML Irrigation_Types_In_Each_Cell() {
...
typedef struct veg_lib_struct veg_lib_struct;
typedef struct {
